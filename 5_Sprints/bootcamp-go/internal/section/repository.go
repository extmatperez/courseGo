...
func (r *repository) Get(ctx context.Context, id int) (domain.Section, error) {
	...
	sqlStatement := `SELECT * FROM "main"."sections" WHERE section_number=$1;`
...
func (r *repository) Update(ctx context.Context, s domain.Section, id int) error {
	stmt, err := r.db.Prepare(`UPDATE "main"."sections" SET "section_number"=?, "current_temperature"=?, "minimum_temperature"=?, "current_capacity"=?, "minimum_capacity"=?, "maximum_capacity"=?, "warehouse_id"=?, "product_type_id"=?  WHERE "section_number"=?;`)
	if err != nil {
		return err
	}
	res, err := stmt.Exec(&s.SectionNumber, &s.CurrentTemperature, &s.MinimumTemperature, &s.CurrentCapacity, &s.MinimumCapacity, &s.MaximumCapacity, &s.WarehouseID, &s.ProductTypeID, &id)
	if err != nil {
		return err
	}
...
func (r *repository) Delete(ctx context.Context, id int) error {
	stmt, err := r.db.Prepare(`DELETE FROM "main"."sections" WHERE section_number=?`)
	if err != nil {
		return err
	}
