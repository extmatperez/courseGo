...
import (
	...
	"github.com/extmatperez/w2GoPrueba/GoStorage/Clase1TM/proyecto/pkg/store"
)
...
func TestGetOneServiceSQL(t *testing.T) {
	...
	personaDB := service.GetOne(7)

	assert.Equal(t, personaNueva.Nombre, personaDB.Nombre)
	assert.Equal(t, personaNueva.Apellido, personaDB.Apellido)
	// assert.Nil(t, misPersonas)
}
...
func TestUpdateServiceSQL(t *testing.T) {
	...
	personaActualizada, _ := service.Update(personaUpdate)
	//personaCreada, _ := service.Store(personaNueva.Nombre, personaNueva.Apellido, personaNueva.Edad)

	// fmt.Println(personaActualizada)
	assert.Equal(t, personaUpdate.Nombre, personaActualizada.Nombre)
	assert.Equal(t, personaUpdate.Apellido, personaActualizada.Apellido)
	service.Update(personaAnterior)
	//assert.Equal(t, 1, personaActualizada.ID)
	// assert.Nil(t, misPersonas)
}

func TestUpdateServiceNotFoundSQL(t *testing.T) {
	//Arrange
	personaUpdate := models.Persona{
		ID:       -15,
		Nombre:   "Cristian",
...
func TestDeletePersonaServiceSQL(t *testing.T) {
	//Arrange
...
func TestDeletePersonaNotFoundServiceSQL(t *testing.T) {
	//Arrange
